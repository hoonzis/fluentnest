...
namespace FluentNest.Tests
{
    public class HistogramTests : TestsBase
    {
        ...
        public void MonthlyHistogramPerCarType()
        {
            ...
            var container = histogram.Aggs.AsContainer<Car>();

            var carTypes = container.GetDictionary(x => x.CarType, v => v.GetDateHistogram<Car>(f => f.Timestamp));
        ...
        public void SumInMonthlyHistogram()
        {
            ...
            // Now with FluentNest
            result = Client.Search<Car>(sc => sc.Aggregations(agg => agg
                .SumBy(x=>x.Price)
