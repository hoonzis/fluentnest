...
namespace FluentNest
{
    public class AggsContainer<T>
    {
        ...
        public IEnumerable<KeyItem> GetGroupBy(Expression<Func<T, object>> fieldGetter)
        {
            return aggs.GetGroupBy(fieldGetter);
        }

        public IEnumerable<TO> GetGroupBy<TO>(Expression<Func<T, object>> fieldGetter, Func<KeyItem, TO> objectTransformer)
        {
            var buckets = aggs.GetGroupBy(fieldGetter);
            return buckets.Select(objectTransformer);
        }

        public IDictionary<TK, TV> GetDictionary<TK, TV>(Expression<Func<T, TK>> keyGetter, Func<KeyItem, TV> objectTransformer)
        {
        ...
        public IDictionary<TK, KeyItem> GetDictionary<TK>(Expression<Func<T, TK>> keyGetter)
        {
            var aggName = keyGetter.GetAggName(AggType.GroupBy);
            var buckets = aggs.GetGroupBy(aggName);
